...
boolean isSelectiveDisplay = false; 
...
MultiplierSlider activeMultInput = null; // Track focus for new sliders

void setup() {
  size(1600, 1000, P3D); 
  smooth(8);
  
  object4D = new Tesseract(); 
  ...
  // Multipliers (Positioned further right: +500)
...
void resetSimulation() {
  ...
  isSelectiveDisplay = false;
  selectiveBtn.label = "Selective: OFF";
...
void draw() {
  ...
  boolean isLog = (object4D instanceof FourDSpiral || object4D instanceof FourDSpiralShell);
...
void mousePressed() {
  ...
  boolean canSelect = (object4D instanceof FourDSpiral || object4D instanceof FourDSpiralShell);
